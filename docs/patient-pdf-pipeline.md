...
- **Geração server-side (Node)**: `lib/pdf-generator.ts` expõe `generatePdfFromText(text, documentId, fileName)` usando `pdf-lib` com paginação, cabeçalho e fallback resiliente. Sanitiza nulos e normaliza quebras de linha antes de gerar.【F:lib/pdf-generator.ts†L1-L98】【F:lib/pdf-generator.ts†L100-L146】
- **Endpoints prontos**:
...
- **Geração client-side**: `ProcessedDocumentViewer` (modal “Visualizar”) cria um PDF local com `pdf-lib` a partir de `clean_text`, permitindo reprocessar, baixar e imprimir sem depender de `pdf_url` ou da rota de pré-visualização.【F:components/patient/processed-document-viewer.tsx†L1-L188】
...
2. **Paginação e layout (pdf-lib)**
   - Use uma fonte incorporada (`StandardFonts.Helvetica`) e calcule quebra de linha com `font.widthOfTextAtSize(...)` para respeitar a largura útil (`pageWidth - margin*2`).【F:lib/pdf-generator.ts†L32-L83】

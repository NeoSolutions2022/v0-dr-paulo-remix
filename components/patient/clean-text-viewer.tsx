...
import { parseMarkdownToStructured } from "@/lib/parsers/clean-text"
...
export default function CleanTextViewer({ markdown }: CleanTextViewerProps) {
  const [search, setSearch] = useState("")
  const [copiedId, setCopiedId] = useState<string | null>(null)
  const structured = useMemo(() => parseMarkdownToStructured(markdown), [markdown])
            ...
            <div className="space-y-4 border-l border-dashed border-slate-200 pl-4">
              {displayEvolutions.map((evolution) => {
                ...
                const badges = [
                  ...
                  upperText.includes("IPSS") ? "IPSS" : null,
                          ...
                          <div className="overflow-hidden rounded-md border">
                            ...
                            <div className="divide-y">
                              {evolution.ipss.items.map((item) => (
                                ...
                                >
                                  <span>{item.n}- {item.pergunta}</span>

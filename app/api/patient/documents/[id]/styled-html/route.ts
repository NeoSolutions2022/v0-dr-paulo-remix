...
function buildFallbackHtml(patientName: string, sanitizedText: string) {
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const finalHtml = html || buildFallbackHtml(patientName, sanitizedText)

  try {
    return NextResponse.json(
      { html: finalHtml, documentHash },
      {
        status: 200,
        headers: {
    ...
    // Mesmo em caso de falha na serialização, retornamos um fallback mínimo para evitar 5xx recorrentes
    return NextResponse.json(
      { html: buildFallbackHtml(patientName, sanitizedText), documentHash },
      {
        status: 200,
        headers: {
          "Cache-Control": "no-cache, no-store, must-revalidate",
